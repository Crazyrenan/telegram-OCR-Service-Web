...
<div class="container mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('requests.create') }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
            + New Request
        </a>
    </div>
            ...
            <tbody class="text-gray-700">
                @forelse($requests as $request)
                    <tr class="hover:bg-gray-100">
                        ...
                        <td class="py-3 px-4 text-center">
                            {{-- Conditionally show the Resend button only for pending requests --}}
                            @if($request->status == 'pending')
                                <form action="{{ route('requests.resend', $request->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn-secondary text-xs px-3 py-2 whitespace-nowrap">Resend Alert</button>
                                </form>
                            @else
